...
@section('content')
    <form method="POST" action="{{ route('posts.store') }}">
     ...
     <p>   
        ...
        faccilitate the correction of it by user.Function old('argument must match input name').
        REMEMBER THAT THE KEY'S AT public function rules() (for validation) at StorePost.php MUST MATCH INPUT NAME.
        Also ALTHOUGH old() parameter key and public function rules() key names are the same, they are not related--}}
     </p>

     <p>   
        <label>Content</label>
        <input type="text" name="content" value="{{old('content')}}"/>{{-- Read comments above --}}
     </p>
